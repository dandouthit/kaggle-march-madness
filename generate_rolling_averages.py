...
rollingAvgCols = ['Season', 'TeamID', 'ScoreAvg', 'AstAvg', 'TOAvg', 'StlAvg', 'BlkAvg', 'ORbAvg', 'DRbAvg', 'PFAvg', 'FGPAvg', '3PPAvg', 'FTPAvg']
dummy_cols = ['Season', 'TeamID', 'DayNum', 'Score', 'Ast', 'TO', 'Stl', 'Blk', 'OR', 'DR', 'PF', 'FGP', '3PP', 'FTP']
...
df_season_data.drop(labels=['WLoc', 'NumOT'], inplace=True, axis=1)

# Combine field goals and free throws into percentages
# Drop the individual field goal and free throw columns after combining them
...
teamList = np.unique(df_season_data[['WTeamID', 'LTeamID']].values)
	...
	for teamID in teamList:
		...
		for ii, row in df_team_season.iterrows():
			if row.WTeamID == teamID:
				df_dummy.loc[ii] = [row.Season, row.WTeamID, row.DayNum, row.WScore, row.WAst, row.WTO, row.WStl, row.WBlk, row.WOR, row.WDR, row.WPF, row.WFGP, row.W3PP, row.WFTP]
			else:
				df_dummy.loc[ii] = [row.Season, row.LTeamID, row.DayNum, row.LScore, row.LAst, row.LTO, row.LStl, row.LBlk, row.LOR, row.LDR, row.LPF, row.LFGP, row.L3PP, row.LFTP]
		...
		df_dummy['BlkAvg'] = df_dummy['Blk'].rolling(numAvgGames).mean()
		df_dummy['ORbAvg'] = df_dummy['OR'].rolling(numAvgGames).mean()
		df_dummy['DRbAvg'] = df_dummy['DR'].rolling(numAvgGames).mean()
		df_dummy['PFAvg'] = df_dummy['PF'].rolling(numAvgGames).mean()
		...
		df_dummy.drop(labels=['DayNum', 'Score', 'Ast', 'TO', 'Stl', 'Blk', 'OR', 'DR', 'PF', 'FGP', '3PP', 'FTP'], inplace=True, axis=1)
